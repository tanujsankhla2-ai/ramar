document.addEventListener("DOMContentLoaded", () => {
    ...
    if (bookingForm) {
        bookingForm.addEventListener('submit', (e) => {
            ...
            const message = `Hello Namastey Salon! I'd like to book a session at your Namastey Salon:%0A%0A*Name:* ${name}%0A*Phone:* ${phone}%0A*Service:* ${service}%0A*Date:* ${date}%0A*Time:* ${time}`;
            ...
            if (serviceBanner) serviceBanner.style.display = 'none';
            alert('Redirecting to WhatsApp to book your Namastey Salon session!');
        });
    }
    ...
    // Rating widget logic: save user's rating per-page in localStorage
    (function setupRatingWidget() {
        const stars = document.querySelectorAll('#rating .star');
        const feedback = document.getElementById('rating-feedback');
        if (!stars || stars.length === 0) return;

        const pageKey = 'rating_' + (location.pathname || 'home');
        let currentRating = 0;
        try {
            const saved = JSON.parse(localStorage.getItem(pageKey));
            if (saved && saved.value) currentRating = Number(saved.value);
        ...
        function render(r) {
            stars.forEach(s => {
                ...
                s.setAttribute('aria-checked', String(val === r));
            });
            if (feedback) {
                feedback.textContent = r ? `Thanks! You rated ${r}/5` : 'Tap a star to rate';
            }
        }
        ...
        stars.forEach(s => {
            ...
            s.addEventListener('mouseleave', () => render(currentRating));
            s.addEventListener('click', () => {
                currentRating = val;
                localStorage.setItem(pageKey, JSON.stringify({ value: currentRating, ts: Date.now() }));
                render(currentRating);
            });
